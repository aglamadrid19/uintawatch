...
- **Round 5 — website slide rebuilt (user: "slide before the end looks bad")**: old card overlapped the phone column during crossfades and its crop read as a random canyon photo. Now the FULL site screenshot (`cards/webfull.png`, 1100x619) in a centered browser card (chrome bar w/ traffic lights + `uintawatch.com` URL pill, frame ring, shadow) at (410,235), "Published in public" centered above, URL line below. Joins into webcard + end card are `fadewhite` (fade through white) — plain crossfade ghosts the card across the phone. Widget blob in the screenshot's bottom-right painted out. Same 60.0s runtime.
- **GitHub QA**: pushed to `main` (raw URL serves 200, sha256 identical to local, video loads/seeks in browser at 1920x1080).
- **Round 3 — report pacing**: E3's type→submit→dialog action happens in only ~1.2s of source (0 chars @9.0s, 16 @9.6s, dialog already up @10.2s — verified with output-side seek). Report flow is now THREE segments: empty form (3.0–6.0 trim, 3.0s), typing + slow-mo spring (9.1–10.5 trim @2x, 2.8s), dialog hold (10.5–14.7 trim, 4.2s). QA this file with output-side seeking (`-i f -ss T`); input-side `-ss` is unreliable on these VFR recordings.
- **Round 4 — pre-form screen**: user flagged that the map screen before the form disappears too fast (form pops at raw 4.0–4.5s). report-a extended to trim 1.8–6.2 @1.2x (5.28s) → map lingers ~2.6s, slide reads, form settles ~1.5s before typing. Video total 60.0s.
